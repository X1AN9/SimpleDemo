...
+ function($) {
  ...
  //如果参数过多，建议通过 config 对象进行配置，而不是传入多个参数。
  $.login = function(text, title, onOK, onCancel, username) {
    var config;
    if (typeof text === 'object') {
      config = text;
    } else {
    ...
    var modal = $.modal({
      text: '<p class="weui-prompt-text">'+(config.text || '')+'</p>' +
            '<input type="text" class="weui-input weui-prompt-input" id="weui-prompt-username" value="' + (config.username || '') + '" placeholder="请输入类型名称" />',
      title: config.title,
      autoClose: false,
      buttons: [
      {
        ...
        onClick: function() {
          ...
          var url='addtype?typename='+username;
    	  window.location.href=url;
          $.closeModal();//关闭添加页面窗口
          config.onOK && config.onOK.call(modal, username);//提示添加成功
        }
      }]
    }, function () {
